...
class CaptionsRunner:
    ...
    def __call__(self, command: list[str], **kwargs) -> subprocess.CompletedProcess[str]:
        ...
        if self.failure is not None:
            return subprocess.CompletedProcess(command, 1, self.stdout, self.failure)

        output = Path(command[command.index("-o") + 1])
        output.parent.mkdir(parents=True, exist_ok=True)
        if self.manual is not None:
            shutil.copyfile(self.manual, output.with_name("captions.en.vtt"))
        if self.automatic is not None:
            shutil.copyfile(self.automatic, output.with_name("captions.en-orig.json3"))
