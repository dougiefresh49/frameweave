...
- Sibling repos are not read by delegates. The fleet repo and voice-lab were sources for the plan and are cited in `docs/`; the earlier tool's fork on disk is never read by anyone, delegate or lead, and its name is on the scrub list.
