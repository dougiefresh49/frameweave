...
#   bakeoff.py --lane codex:gpt-5.6-luna --frames DIR --out results/LANE.jsonl [--batch N] [--effort low]
...
PROMPT = (
    ...
    "Return JSON only, no prose, no code fence: a list with one object per image in the order given, "
    ...
    "file names, tab titles, captions>], \"illegible\": <true if any visible text is too small or blurred to read with certainty>}}. "
    "Rules: transcribe, never infer or complete; if unsure of a single character, leave that string out rather than guess; "
    "an empty list is the right answer when there is no readable text. Every frame index must appear exactly once."
)
...
def run_claude(model, images, bare=False):
    ...
    try:
        ...
        raw = d.get("result", "")
        u = d.get("usage", {})
        tokens_in = (u.get("input_tokens", 0) or 0) + (u.get("cache_read_input_tokens", 0) or 0) + (u.get("cache_creation_input_tokens", 0) or 0)
        tokens_out = u.get("output_tokens")
        cost = d.get("total_cost_usd")
    except json.JSONDecodeError:
        pass
    return dict(seconds=secs, tokens_in=tokens_in, tokens_out=tokens_out, tokens_total=(tokens_in or 0) + (tokens_out or 0) or None, cost_usd_reported=cost, raw=raw, err=p.stderr[-2000:], rc=p.returncode)
...
def run_gemini(model, images):
    ...
    client = genai.Client()
    parts = [types.Part.from_bytes(data=Path(im).read_bytes(), mime_type="image/jpeg") for im in images]
    ...
    secs = time.time() - t
    u = r.usage_metadata
    return dict(seconds=secs, tokens_in=u.prompt_token_count, tokens_out=u.candidates_token_count,
                tokens_total=u.total_token_count, raw=r.text or "", err="", rc=0)
...
def main():
    ...
    with out.open("w") as fh:
        fh.write(json.dumps({"meta": True, "lane": a.lane, "batch": a.batch, "effort": a.effort, "usage_before": before, "started": time.strftime("%Y-%m-%dT%H:%M:%S")}) + "\n")
        for i in range(0, len(frames), a.batch):
            ...
            print(a.lane, r["frames"], f"{r.get('seconds') or 0:.1f}s", "tokens", r.get("tokens_total"), "parsed" if r["parsed"] else "NOPARSE", flush=True)
        after = usage_reading()
        fh.write(json.dumps({"meta": True, "usage_after": after, "finished": time.strftime("%Y-%m-%dT%H:%M:%S")}) + "\n")
