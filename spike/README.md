Folded onto main from branch `spike/provider-fit` on 2026-09-24. Backlog item 21 (issue #21): the evidence behind decisions 47 to 49 and the seed coefficients in `src/frameweave/lanes.toml`. Nothing imports it.
...
- `labels.json`: 20 hand-labeled frames from the three sample videos (decision 27). The frames themselves are not in git; they were kept on the owner's machine, copied from the sample runs' existing 1280x720 frames.
